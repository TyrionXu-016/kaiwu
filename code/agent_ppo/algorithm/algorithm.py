...
class Algorithm:
    ...
    def learn(self, list_sample_data):
        ...
        # Per-batch advantage normalization (standard PPO stability).
        # 按 batch 对优势做归一化，减小尺度波动带来的梯度不稳定。
        if advantage.shape[0] > 1:
            adv_shape = advantage.shape
            adv = advantage.reshape(adv_shape[0], -1)
            adv_std = adv.std(dim=0, unbiased=False, keepdim=True).clamp(min=1e-8)
            adv = (adv - adv.mean(dim=0, keepdim=True)) / adv_std
            advantage = adv.view(adv_shape)
